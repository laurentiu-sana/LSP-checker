...
public class AttributedString {
    ...
    static class AttributedIterator implements AttributedCharacterIterator {
        ...
        private int runLimit(List<Range> ranges) {
            int result = end;
            ListIterator<Range> it = (ListIterator<Range>) ranges.listIterator(ranges.size());
    ...
    public AttributedString(AttributedCharacterIterator iterator) {
        if (iterator.getBeginIndex() > iterator.getEndIndex()) {
            ...
            throw new IllegalArgumentException("text.0A"); //$NON-NLS-1$
        }
        ...
        //attributeMap = HashMap<Attribute, List<Range>>(0,1);
        //attributeMap = new HashMap<Attribute, List<Range>>(
        //        (attributes.size() * 4 / 3) + 1);

        Iterator<lsp.test.jdk.util.AttributedCharacterIterator.Attribute> it = attributes.iterator();
        while (it.hasNext()) {
            AttributedCharacterIterator.Attribute attribute = it.next();
            iterator.setIndex(0);
    ...
    public AttributedString(String value,
        ...
        if (value.length() == 0 && !attributes.isEmpty()) {
            ...
            throw new IllegalArgumentException("text.0B"); //$NON-NLS-1$
        }
        text = value;
        //attributeMap = new HashMap<Attribute, List<Range>>(
        //        (attributes.size() * 4 / 3) + 1);
    ...
    public void addAttribute(AttributedCharacterIterator.Attribute attribute,
        ...
        lsp.test.jdk.util.ListIterator<Range> it = ranges.listIterator();
        while (it.hasNext()) {
            Range range = it.next();
            if (end <= range.start) {
                it.previous();
