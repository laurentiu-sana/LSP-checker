    ...
    */
    ...
    public SimpleTimeZone(int offset, String name, int startMonth,
        ...
        if (daylightSavings <= 0) {
            throw new IllegalArgumentException("luni.3B"); //$NON-NLS-1$
        }
    ...
    public int getOffset(int era, int year, int month, int day, int dayOfWeek,
        ...
        if (era != GregorianCalendar.BC && era != GregorianCalendar.AD) {
            throw new IllegalArgumentException("luni.3C"); //$NON-NLS-1$
        }
    ...
    private void checkRange(int month, int dayOfWeek, int time) {
        if (month < Calendar.JANUARY || month > Calendar.DECEMBER) {
            throw new IllegalArgumentException("luni.3D"); //$NON-NLS-1$
        }
        if (dayOfWeek < Calendar.SUNDAY || dayOfWeek > Calendar.SATURDAY) {
            throw new IllegalArgumentException("luni.48"); //$NON-NLS-1$
        }
        if (time < 0 || time >= 24 * 3600000) {
            throw new IllegalArgumentException("luni.3E"); //$NON-NLS-1$
        }
    }

    private void checkDay(int month, int day) {
        if (day <= 0 || day > GregorianCalendar.DaysInMonth[month]) {
            throw new IllegalArgumentException("luni.3F"); //$NON-NLS-1$
        }
    }
        ...
        if (endDay != 0) {
            ...
            if (endMode != DOW_IN_MONTH_MODE) {
                ...
                if (endDay < -5 || endDay > 5) {
                    throw new IllegalArgumentException("luni.40"); //$NON-NLS-1$
                }
            }
        }
        if (endMode != DOM_MODE) {
